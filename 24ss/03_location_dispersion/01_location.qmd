...
[TraiNex](https://trainex42.de/hmu24)
...
[**Fall 1: Populationskennwerte**]{.underline color="#00a216"}

Vollständige Population liegt vor (z.B. alle US-Präsidenten) <br>&rArr; Bestimmung exakter statistische Kennwerte für die Population<br>&rArr; *Populationskennwerte* (z.B. Populationsstreuung)<br>[(Hinweis: man spricht auch manchmal von Populations*parametern*)]{style="font-size:20px"}

<div class="vspace-xsmall"></div>

[**Fall 2: Stichprobenkennwerte**]{.underline color="#000db2"}

Stichprobe liegt vor<br>&rArr; Ziel ist Beschreibung der Stichprobe *ohne Inferenz auf die Population*<br>&rArr; *Stichprobenkennwerte* (z.B. Stichprobenstreuung)

<div class="vspace-xsmall"></div>

[**Fall 3: Inferentielle Kennwerte**]{.underline color="#c89f00"}

Stichprobe liegt vor<br>&rArr; Ziel ist *Inferenz auf die Population*, d.h. die Populationskennwerte sollen auf Basis der Stichprobe geschätzt werden.<br>&rArr; *Inferentielle Kennwerte* (z.B. inferentielle Streuung)
...
![](images/population_stichprobe_inferenz.png)
...
- Folgendes Schema hat sich in der Literatur etabliert:
  - [Populationskennwerte]{color="#00a216"}: griechische Buchstaben
  - [Stichprobenkennwerte]{color="#000db2"}: lateinische Buchstaben
  - [Inferentielle Kennwerte]{color="#c89f00"}: griechische Buchstaben mit Zirkumflex (^).

:::{.fragment}
<!---  Table --->
||[Populations-\
Kennwert]{color="#00a216"}|[Stichproben-\
Kennwert]{color="#000db2"}|[Inferentieller\
Kennwert]{color="#c89f00"}|
|-|-|-|-|
| Mittelwert | $\mu$ | $\bar{x}$, $m$, $M$ | $\hat{\mu}$ |
| Standardabweichung |$\sigma$ | $s$ |  $\hat{\sigma}$ |
| Varianz |$\sigma^2$ | $s^2$, $Var(X)$ | $\hat{\sigma}^2$, $\hat{Var}(X)$ |
| Kovarianz |$\sigma_{\mkern-2mu\scriptscriptstyle{XY}}$ | $s_{\mkern-2mu\scriptscriptstyle{XY}}$, $Cov(X,Y)$ | $\hat{\sigma}_{\mkern-2mu\scriptscriptstyle{XY}}$, $\hat{Cov}(X,Y)$ |
| Korrelation |$\rho$| $r$ | $\hat{\rho}$ |

: {tbl-colwidths="[20, 19, 19, 19]"}
:::
...
::: {.colorbox .fragment}
Um die Komplexität zu reduzieren, werden wir **in dieser Vorlesungsreihe konsequent Formeln für inferentielle Kennwerte behandeln** (zu erkennen am ^). Dies ist der mit Abstand häufigste Fall in der Psychologie und de facto in allen empirischen Wissenschaften.
:::
...
::: {.column width="30%"}
:::{.fragment}
![](images/rechtsschief_linksschief.png){height=200px}
:::
:::
::::
